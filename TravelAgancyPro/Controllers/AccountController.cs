...
using TravelAgancyPro.Providers;
using TravelAgancyPro.Results;
...
namespace TravelAgancyPro.Controllers
{
    ...
    public class AccountController : ApiController
    {
        ...
        [AllowAnonymous]
        public async Task<IHttpActionResult> ForgetPassword(string Email, string Password, string ConfirmPassword)
        {
            if (!ModelState.IsValid)
            {
                return BadRequest(ModelState);
            ...
            if (Password != ConfirmPassword)
                return BadRequest("Password Does Not Match ConfirmPassword");
            ...
            using (TravelAgancyEntities db = new TravelAgancyEntities())
            {
                ...
                userId = await db.AspNetUsers.Where(a => a.Email == Email).Select(b => b.Id).SingleOrDefaultAsync();
        ...
        [AllowAnonymous]
        [Route("Register")]
        public async Task<IHttpActionResult> Register(RegisterBindingModel model, string Rolename, string TargetRole)
        {
            if (!ModelState.IsValid)
            {
                return BadRequest(ModelState);
            ...
            if (Rolename == RoleName.Admin)
            {
                ...
                else if (TargetRole == RoleName.Manger)
                {
                    UserManager.AddToRole(user.Id, RoleName.Manger);

                }
                else if (TargetRole == RoleName.User)
                {
